...
struct LobbyInfo {
    ...
    LobbyInfo() = default;
    LobbyInfo(const std::string& id, const std::string& lobbyName) : lobbyId(id), name(lobbyName) {}
...
struct ServerInfo {
    ...
    // Connection string
    std::string getConnectionString() const { return ip + ":" + std::to_string(port); }
...
struct ServerList {
    ...
    std::vector<ServerInfo> getFiltered() const {
        ...
        for (const auto& server : servers) {
            if (!showFull && !server.hasSpace())
                continue;
            if (!showEmpty && server.currentPlayers == 0)
                continue;
            if (!showPasswordProtected && server.isPasswordProtected)
                continue;
            if (showOfficialOnly && !server.isOfficial)
                continue;
