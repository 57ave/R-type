...
public:
    ...
    using EnemySpawnCallback = std::function<ECS::Entity(const LuaEnemyConfig&, float x, float y, const std::string& pattern)>;
    using BossSpawnCallback = std::function<ECS::Entity(const LuaBossConfig&)>;
    using PowerUpSpawnCallback = std::function<ECS::Entity(const std::string& type, float x, float y)>;
    using ProjectileSpawnCallback = std::function<ECS::Entity(const LuaWeaponConfig&, float x, float y, float angle, bool isPlayer)>;
    ...
    static LuaWeaponConfig GetWeaponConfig(sol::state& lua, const std::string& weaponName, int level = 1);
