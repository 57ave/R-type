...
public:
    ...
    void Init() override { std::cout << "[NetworkSystem] Initialized" << std::endl; }
    ...
    void setLocalPlayerId(uint8_t playerId) { localPlayerId_ = playerId; }
...
private:
    ...
    void handleServerWelcome(const NetworkPacket& packet) {
        if (packet.payload.size() >= 1) {
            ...
            std::cout << "[NetworkSystem] Received SERVER_WELCOME. Player ID: "
                      << (int)localPlayerId_ << std::endl;
        }
    }
...
private:
    ...
    void handleRoomListReply(const NetworkPacket& packet) {
        ...
        try {
            ...
            std::cerr << "[NetworkSystem] Error deserializing ROOM_LIST_REPLY: " << e.what()
                      << std::endl;
            RType::Network::NetworkBindings::OnRoomListReceived({});
        }
    }
    ...
    void handleRoomCreated(const NetworkPacket& packet) {
        ...
        try {
            ...
            std::cerr << "[NetworkSystem] Error deserializing ROOM_CREATED: " << e.what()
                      << std::endl;
        }
    }
    ...
    void handleRoomJoined(const NetworkPacket& packet) {
        ...
        if (packet.payload.size() < sizeof(uint32_t) + 1) {
            std::cerr << "[NetworkSystem] ROOM_JOINED payload too small: " << packet.payload.size()
                      << " bytes" << std::endl;
            return;
        }
        ...
        try {
            ...
            std::cerr << "[NetworkSystem] Error deserializing ROOM_JOINED: " << e.what()
                      << std::endl;
        }
    }
    ...
    void handleChatMessage(const NetworkPacket& packet) {
        ...
        try {
            ...
            std::cerr << "[NetworkSystem] Error deserializing CHAT_MESSAGE: " << e.what()
                      << std::endl;
        }
    }
    ...
    void createEntityFromState(const EntityState& state) {
        ...
        bool isLocal =
            (state.type == EntityType::ENTITY_PLAYER && state.playerId == localPlayerId_);
        coordinator_->AddComponent(entity,
                                   NetworkId(state.id, isLocal, state.playerId, state.playerLine));

        // Add Position
        coordinator_->AddComponent(
            entity, Position{static_cast<float>(state.x), static_cast<float>(state.y)});

        // Add Velocity
        coordinator_->AddComponent(
            entity, Velocity{static_cast<float>(state.vx), static_cast<float>(state.vy)});
        ...
        switch (state.type) {
            ...
            case EntityType::ENTITY_MONSTER:
                ...
                std::cout << "[NetworkSystem] Created Enemy entity " << entity
                          << " (type: " << (int)state.enemyType << ") at (" << state.x << ", "
                          << state.y << ")" << std::endl;
                break;
            case EntityType::ENTITY_PLAYER_MISSILE:
                ...
                std::cout << "[NetworkSystem] Created PlayerBullet entity " << entity << " at ("
        ...
        std::cout << "[NetworkSystem] Created entity " << entity << " for network ID " << state.id
                  << std::endl;
    }
