...
void NetworkBindings::RequestRoomList() {
    if (!s_networkClient) {
        std::cerr << "[NetworkBindings] Cannot request room list: NetworkClient not set" << std::endl;
        return;
    }
    
    if (!s_networkClient->isConnected()) {
        std::cerr << "[NetworkBindings] Cannot request room list: not connected to server" << std::endl;
        return;
    }
...
void NetworkBindings::LeaveRoom() {
    ...
    std::cout << "[NetworkBindings] LEAVE_ROOM (packet not implemented in protocol yet)" << std::endl;
...
void NetworkBindings::OnRoomListReceived(const std::vector<RoomInfo>& rooms) {
    ...
    if (!result.valid()) {
        ...
        std::cerr << "[NetworkBindings] Lua error in OnServerListUpdated: " 
                  << err.what() << std::endl;
    } else {
        std::cout << "[NetworkBindings] Room list sent to Lua (" << rooms.size() << " rooms)" << std::endl;
    }
}
...
void NetworkBindings::OnRoomJoined(uint32_t roomId, const std::string& roomName, uint8_t maxPlayers, bool isHost) {
    ...
    if (!luaCallback.valid()) {
        std::cout << "[NetworkBindings] OnRoomJoined not found in Lua (optional callback)" << std::endl;
        return;
    }
...
void NetworkBindings::OnPlayerJoinedRoom(uint32_t playerId, const std::string& playerName) {
    ...
    if (!luaCallback.valid()) {
        std::cout << "[NetworkBindings] OnPlayerJoined not found in Lua (optional callback)" << std::endl;
        return;
    }
...
void NetworkBindings::OnPlayerLeftRoom(uint32_t playerId) {
    ...
    if (!luaCallback.valid()) {
        std::cout << "[NetworkBindings] OnPlayerLeft not found in Lua (optional callback)" << std::endl;
        return;
    }
...
void NetworkBindings::OnPlayerReadyChanged(uint32_t playerId, bool ready) {
    ...
    if (!luaCallback.valid()) {
        std::cout << "[NetworkBindings] OnPlayerReadyChanged not found in Lua (optional callback)" << std::endl;
        return;
    }
    ...
    if (!result.valid()) {
        sol::error err = result;
        std::cerr << "[NetworkBindings] Lua error in OnPlayerReadyChanged: " << err.what() << std::endl;
    }
}
...
void NetworkBindings::OnGameStarting(int countdown) {
    ...
    if (!luaCallback.valid()) {
        std::cout << "[NetworkBindings] OnGameStarting not found in Lua (optional callback)" << std::endl;
        return;
    }
...
void NetworkBindings::OnRoomPlayersUpdated(const std::vector<PlayerInRoomInfo>& players) {
    if (!s_lua) {
        std::cerr << "[NetworkBindings] Cannot call OnRoomPlayersUpdated - Lua state not set" << std::endl;
        return;
    }
    ...
    if (!result.valid()) {
        sol::error err = result;
        std::cerr << "[NetworkBindings] Error calling OnRoomPlayersUpdated: " << err.what() << std::endl;
    } else {
        std::cout << "[NetworkBindings] Room players updated (" << players.size() << " players)" << std::endl;
    }
}
...
void NetworkBindings::SendChatMessage(const std::string& message) {
    ...
    if (!s_networkClient->isConnected()) {
        std::cerr << "[NetworkBindings] Cannot send chat message: not connected to server" << std::endl;
        return;
    }
