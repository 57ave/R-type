...
std::string ResolveAssetPath(const std::string& relativePath) {
    ...
    if (!g_basePath.empty()) {
        if (fs::exists(g_basePath + relativePath))
            return g_basePath + relativePath;
        if (fs::exists(g_basePath + strippedPath))
            return g_basePath + strippedPath;
    }
...
ECS::Entity Game::CreateExplosion(float x, float y) {
    ...
    std::cout << "[CreateExplosion] Explosion " << explosion << " created successfully!"
              << std::endl;
    return explosion;
}
...
int Game::Run(int argc, char* argv[]) {
    ...
    if (argc > 1 && std::string(argv[1]) == "--network") {
        ...
        std::cout << "[Game] Network mode enabled. Server: " << serverAddress << ":" << serverPort
                  << std::endl;
        std::cout << "[Game] *** isNetworkClient = " << (isNetworkClient ? "TRUE" : "FALSE")
                  << " ***" << std::endl;
    } else {
    ...
    std::cout << "[Game] Network bindings registered to Lua (deferred connect supported)"
              << std::endl;
    ...
    collisionSystem->SetCollisionCallback([this, networkMode](ECS::Entity a, ECS::Entity b) {
        ...
        if (networkMode) {
            ...
            std::cout << "[Client] Visual collision detected: " << a << " <-> " << b
                      << " (server authoritative)" << std::endl;
        ...
        if (!aExists || !bExists)
            return;  // Skip if either entity is already destroyed

        // Check if either entity is already marked for destruction
        bool aMarkedForDestruction = std::find(entitiesToDestroy.begin(), entitiesToDestroy.end(),
                                               a) != entitiesToDestroy.end();
        bool bMarkedForDestruction = std::find(entitiesToDestroy.begin(), entitiesToDestroy.end(),
                                               b) != entitiesToDestroy.end();

        if (aMarkedForDestruction || bMarkedForDestruction)
            return;  // Skip duplicate collisions
        ...
        if (aIsPlayer && gCoordinator.HasComponent<Health>(a)) {
            ...
            if (health.invincibilityTimer > 0.0f)
                return;  // Player is invincible, skip collision
        }
        ...
        bool significant =
            (aHasDamage && bHasHealth) || (bHasDamage && aHasHealth) || aIsPlayer || bIsPlayer;
        if (!significant) {
        ...
        if (aHasHealth && bHasDamage) {
            ...
            if (!health.invulnerable) {
                ...
                std::cout << "[Damage] Entity " << a << " took " << damage.amount
                          << " damage, health: " << health.current << std::endl;
            }
        }
        ...
        if (bHasHealth && aHasDamage) {
            ...
            if (!health.invulnerable) {
                health.current -= damage.amount;
                std::cout << "[Damage] Entity " << b << " took " << damage.amount
                          << " damage, health: " << health.current << std::endl;
            }
        }
        ...
        if (aHasHealth) {
            ...
            if (health.current <= 0 && health.destroyOnDeath) {
                ...
                std::cout << "[Player] Hit! Health: " << health.current << "/" << health.max
                          << " - Invincible for " << health.invincibilityDuration << "s"
                          << std::endl;
            }
        }
        if (bHasHealth) {
                ...
                if (projTag.isPlayerProjectile) {
                    ...
                    for (auto entity : allEntities) {
                        if (gCoordinator.HasComponent<ShootEmUp::Components::PlayerTag>(entity) &&
                            ...
                            auto& playerTag =
                                gCoordinator.GetComponent<ShootEmUp::Components::PlayerTag>(entity);
                    ...
                    if (localPlayer != 0) {
                        auto& score =
                            gCoordinator.GetComponent<ShootEmUp::Components::Score>(localPlayer);
                        ...
                        if (gCoordinator.HasComponent<ShootEmUp::Components::EnemyTag>(b)) {
                            auto& enemyTag =
                                gCoordinator.GetComponent<ShootEmUp::Components::EnemyTag>(b);
                            pointsAwarded = enemyTag.scoreValue;
                        }

                        score.AddPoints(pointsAwarded);
                        std::cout << "[Score] +" << pointsAwarded << " points (x"
                                  << score.comboMultiplier << ") | Total: " << score.currentScore
                                  << std::endl;
                    }
                }
            }
    ...
    auto initializeNetworkMode = [&](const std::string& address, short port) {
        ...
        try {
            ...
            networkSystem =
                std::make_shared<eng::engine::systems::NetworkSystem>(&gCoordinator, networkClient);
            ...
            networkSystem->setEntityCreatedCallback([this](ECS::Entity entity) {
                ...
                std::cout << "[Game] 🎨 Creating sprite for network entity " << entity
                          << " (Tag: " << tag.name << ")" << std::endl;
            ...
            networkSystem->setEntityDestroyedCallback(
                [this](ECS::Entity entity, uint32_t networkId) {
                    std::cout << "[Game] Network entity " << entity << " (ID: " << networkId
                              << ") destroyed" << std::endl;
                });
            ...
            while (!connected) {
                ...
                if (networkClient->hasReceivedPackets()) {
                    ...
                    if (static_cast<GamePacketType>(packet.header.type) ==
                        GamePacketType::SERVER_WELCOME) {
                        if (packet.payload.size() >= 1) {
                            uint8_t playerId = static_cast<uint8_t>(packet.payload[0]);
                            networkSystem->setLocalPlayerId(playerId);
                            std::cout << "[Game] ✅ Connected! Player ID: " << (int)playerId
                                      << std::endl;
                            connected = true;
                        }
                    }
    ...
    if (!gameFont.loadFromFile(fontPath)) {
        ...
        if (!gameFont.loadFromFile(fontPath)) {
            std::cerr << "[Game] Warning: Could not load font - UI text will not display"
                      << std::endl;
    ...
    devConsole->registerCommand(
        "health", "Set player health", "health <value>",
        [this](const std::vector<std::string>& args) -> std::string {
            if (args.size() < 2) {
    ...
    std::cout << "[Game] Background split into " << backgroundTiles.size()
              << " tiles (MAX_TEXTURE_SIZE=" << maxTextureSize << ")" << std::endl;
    ...
    if (!enemyBulletTexture->loadFromFile(
            ResolveAssetPath("game/assets/enemies/enemy_bullets.png"))) {
        std::cerr << "Error: Could not load enemy bullet sprite" << std::endl;
        return 1;
    }

    std::cout << "[Game] ✅ Enemy bullet texture loaded: " << enemyBulletTexture->getSize().x << "x"
              << enemyBulletTexture->getSize().y << std::endl;

    explosionTexture = std::make_unique<SFMLTexture>();
    if (!explosionTexture->loadFromFile(
            ResolveAssetPath("game/assets/enemies/r-typesheet44.png"))) {
    ...
    if (!luaState.LoadScript(ResolveAssetPath("assets/scripts/init.lua"))) {
        ...
        if (networkMode) {
            ...
            if (initSolo.valid()) {
                initSolo();
                std::cout << "[Game] Solo mode initialized - Enemy showcase may be active"
                          << std::endl;
            }
        }
    }
    ...
    gameStateCallbacks.goBack = []() {
        ...
        if (state == GameState::Paused) {
            gsm.SetState(GameState::Playing);
        } else if (state == GameState::Options || state == GameState::Credits ||
    ...
    std::cout << "[Game] Asset base path set for Lua: "
              << (g_basePath.empty() ? "(current dir)" : g_basePath) << std::endl;
    ...
    luaState.GetState()["SetMenuMusicVolume"] = [this](float volume) { SetMusicVolume(volume); };

    luaState.GetState()["GetMenuMusicVolume"] = [this]() -> float { return GetMusicVolume(); };

    // New audio control callbacks
    luaState.GetState()["OnMusicVolumeChanged"] = [this](float value) { SetMusicVolume(value); };

    luaState.GetState()["OnSFXVolumeChanged"] = [this](float value) { SetSFXVolume(value); };

    // Save settings callback (called from Lua OnApplySettings)
    luaState.GetState()["SaveUserSettingsToFile"] = [this]() { SaveUserSettings(); };
    ...
    audioNamespace["StopMusic"] = [this]() { StopMusic(); };

    audioNamespace["PauseMusic"] = [this]() { PauseMusic(); };

    audioNamespace["ResumeMusic"] = [this]() { ResumeMusic(); };

    audioNamespace["SetMusicVolume"] = [this](float volume) { SetMusicVolume(volume); };

    audioNamespace["SetSFXVolume"] = [this](float volume) { SetSFXVolume(volume); };

    audioNamespace["GetMusicVolume"] = [this]() -> float { return GetMusicVolume(); };

    audioNamespace["GetSFXVolume"] = [this]() -> float { return GetSFXVolume(); };

    // Stage/Boss music control
    audioNamespace["SetStage"] = [this](int stage) { SetCurrentStage(stage); };

    audioNamespace["OnBossSpawned"] = [this]() { OnBossSpawned(); };

    audioNamespace["OnBossDefeated"] = [this]() { OnBossDefeated(); };

    audioNamespace["OnGameOver"] = [this]() { OnGameOver(); };

    audioNamespace["OnVictory"] = [this]() { OnAllStagesClear(); };
    ...
    if (!backgroundTiles.empty())
        textureMap["background"] = backgroundTiles[0].get();
                    ...
                    if (spriteTbl.valid()) {
                        ...
                        if (!texPath.empty() && textureMap.find(texPath) == textureMap.end()) {
                            ...
                            if (loaded) {
                                ...
                                std::cerr << "[Game] Warning: could not load enemy texture '"
                                          << texPath << "'" << std::endl;
                            }
                        }
                    }
    ...
    RType::Scripting::FactoryBindings::RegisterFactories(
        luaState.GetState(), &gCoordinator, textureMap, &allSprites,
    ...
    spawnScriptSystem = Scripting::ScriptedSystemLoader::LoadSystem(
        ResolveAssetPath("assets/scripts/systems/spawn_system.lua"), &gCoordinator);
    ...
    if (networkMode) {
        std::cout << "[Game] Network mode: Starting on main menu - waiting for room creation/join"
                  << std::endl;
    }
    ...
    while (window.isOpen()) {
        ...
        bool inMenu = (currentState == GameState::MainMenu || currentState == GameState::Paused ||
                       currentState == GameState::Options || currentState == GameState::Lobby ||
            ...
            for (auto entity : allEntities) {
                if (gCoordinator.HasComponent<NetworkId>(entity)) {
                    ...
                    if (netId.isLocalPlayer) {
                        player = entity;
                        std::cout << "[Game] Found local player entity: " << player
                                  << " (networkId: " << netId.networkId << ")" << std::endl;
                        break;
                    }
                }
            }
        ...
        if (currentState == GameState::Playing && !winConditionTriggered) {
            ...
            if (gamePlayTime >= WIN_TIME_THRESHOLD) {
                std::cout << "[Game] 🎉 WIN! Player survived for " << gamePlayTime << " seconds!"
                          << std::endl;
        ...
        if (!networkMode) {
            ...
            if (runtimeClient && runtimeClient->isConnected()) {
                // Initialize network system if needed (first time we have a connected runtime
                // client)
                if (!networkSystem && !networkClient) {
                    std::cout << "[Game] 🌐 Runtime client detected - initializing network mode..."
                              << std::endl;

                    // Assign the runtime client
                    networkClient = runtimeClient;
                    networkSystem = std::make_shared<eng::engine::systems::NetworkSystem>(
                        &gCoordinator, networkClient);
                    ...
                    while (!gotPlayerId) {
                        ...
                        if (networkClient->hasReceivedPackets()) {
                            ...
                            if (static_cast<GamePacketType>(packet.header.type) ==
                                ...
                                if (packet.payload.size() >= 1) {
                                    uint8_t playerId = static_cast<uint8_t>(packet.payload[0]);
                                    networkSystem->setLocalPlayerId(playerId);
                                    std::cout << "[Game] ✅ Received player ID: " << (int)playerId
                                              << std::endl;
                                    gotPlayerId = true;
                                }
                            }
                        ...
                        auto now = std::chrono::steady_clock::now();
                        if (std::chrono::duration_cast<std::chrono::seconds>(now - startTime)
                                .count() > 3) {
                            std::cout
                                << "[Game] ⏱️  Timeout waiting for player ID (will continue anyway)"
                                << std::endl;
                            break;
                        }
                    ...
                    std::cout << "[Game] ✅ Network mode fully initialized from runtime connection!"
                              << std::endl;
                }
        ...
        if (activeClient) {
            ...
            while (activeClient->hasReceivedPackets()) {
                ...
                std::cout << "[Game] Received packet type: " << static_cast<int>(packet.header.type)
                          << std::endl;
                ...
                switch (packetType) {
                    case GamePacketType::ROOM_LIST_REPLY: {
                        try {
                            ...
                            std::cout << "[Game] Received room list: " << payload.rooms.size()
                                      << " rooms" << std::endl;
                        } catch (const std::exception& e) {
                            std::cerr << "[Game] Error parsing ROOM_LIST_REPLY: " << e.what()
                                      << std::endl;
                        }
                        break;
                    }
                    ...
                    case GamePacketType::ROOM_JOINED: {
                        if (packet.payload.size() >= 4) {
                            try {
                                ...
                                bool isHost = (networkSystem &&
                                               hostPlayerId == networkSystem->getLocalPlayerId());
                                RType::Network::NetworkBindings::OnRoomJoined(roomId, roomName,
                                                                              maxPlayers, isHost);
                                std::cout << "[Game] Successfully joined room: " << roomName
                                          << " (ID: " << roomId
                                          << ", max: " << static_cast<int>(maxPlayers)
                                          << ", host: " << (isHost ? "YES" : "NO") << ")"
                                          << std::endl;
                            } catch (const std::exception& e) {
                                std::cerr << "[Game] Error parsing ROOM_JOINED: " << e.what()
                                          << std::endl;
                            }
                        }
                        break;
                    ...
                    case GamePacketType::ROOM_PLAYERS_UPDATE: {
                        try {
                            RoomPlayersPayload payload =
                                RoomPlayersPayload::deserialize(packet.payload);
                            std::cout << "[Game] Received player list update for room "
                                      << payload.roomId << ": " << payload.players.size()
                                      << " players" << std::endl;
                            RType::Network::NetworkBindings::OnRoomPlayersUpdated(payload.players);
                        } catch (const std::exception& e) {
                            std::cerr << "[Game] Error parsing ROOM_PLAYERS_UPDATE: " << e.what()
                                      << std::endl;
                        }
                        break;
                    }
                    ...
                    case GamePacketType::CHAT_MESSAGE: {
                        try {
                            ChatMessagePayload payload =
                                ChatMessagePayload::deserialize(packet.payload);
                            std::cout << "[Game] Chat from " << payload.senderName << ": "
                                      << payload.message << std::endl;
                            RType::Network::NetworkBindings::OnChatMessage(payload.senderName,
                                                                           payload.message);
                        } catch (const std::exception& e) {
                            std::cerr << "[Game] Error parsing CHAT_MESSAGE: " << e.what()
                                      << std::endl;
                        }
                        break;
                    }
                    ...
                    default:
                        // Unknown packet type
                        std::cout << "[Game] Unknown/unhandled packet type: " << packet.header.type
                                  << std::endl;
                        break;
                }
            }
        ...
        if (networkMode && networkSystem) {
            ...
            if (frameCounter++ % 60 == 0) {  // Log every 60 frames
                std::cout << "[Game] Enemies: " << enemyCount << " total, " << enemyWithSpriteCount
                          << " with sprites, " << (enemyCount - enemyWithSpriteCount)
                          << " invisible" << std::endl;
            }
            ...
            for (auto entity : allEntities) {
                if (entitiesWithSprites.find(entity) == entitiesWithSprites.end() &&
                    !gCoordinator.HasComponent<Sprite>(
                        entity) &&  // IMPORTANT: Don't add sprite if already exists
                    ...
                    std::cout << "[Game] Adding sprite to entity " << entity
                              << " with tag: " << tag.name << " at position (" << pos.x << ", "
                              << pos.y << ")" << std::endl;
                    ...
                    if (tag.name == "Player") {
                        ...
                        std::cout << "[Game] Enemy entity " << entity << " set to scale 2.5x"
                                  << std::endl;
                    ...
                    } else if (tag.name == "PlayerBullet" || tag.name == "bullet" ||
                               tag.name == "charged_bullet") {
                        ...
                        if (gCoordinator.HasComponent<ShootEmUp::Components::ProjectileTag>(
                                entity)) {
                            auto& projTag =
                                gCoordinator.GetComponent<ShootEmUp::Components::ProjectileTag>(
                                    entity);
                        ...
                        std::cout << "[Game] Added explosion sprite to network entity " << entity
                                  << std::endl;

                    } else {
                        // Unknown entity type - log warning and use default enemy appearance
                        std::cerr << "[Game] WARNING: Unknown entity tag '" << tag.name
                    ...
                    std::cout << "[Game] Added sprite to network entity " << entity << " ("
                              << tag.name << ") scale=" << spriteComp.scaleX << "x"
                              << spriteComp.scaleY << std::endl;
                }
            }
        }
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == eng::engine::EventType::KeyReleased &&
                event.key.code == eng::engine::Key::F1) {
                showDevConsole = !showDevConsole;
                std::cout << "[DevTools] Console " << (showDevConsole ? "opened" : "closed")
                          << std::endl;
            }

            // F2 to toggle profiler overlay
            if (event.type == eng::engine::EventType::KeyReleased &&
                event.key.code == eng::engine::Key::F2) {
                showProfiler = !showProfiler;
                std::cout << "[DevTools] Profiler " << (showProfiler ? "opened" : "closed")
                          << std::endl;
            }
            ...
            if (showDevConsole && devConsole) {
                if (event.type == eng::engine::EventType::TextEntered) {
                    ...
                    if (event.key.code == eng::engine::Key::Enter)
                        sfmlKey = sf::Keyboard::Return;
                    else if (event.key.code == eng::engine::Key::Backspace)
                        sfmlKey = sf::Keyboard::BackSpace;
                    else if (event.key.code == eng::engine::Key::Up)
                        sfmlKey = sf::Keyboard::Up;
                    else if (event.key.code == eng::engine::Key::Down)
                        sfmlKey = sf::Keyboard::Down;
                    else if (event.key.code == eng::engine::Key::Tab)
                        sfmlKey = sf::Keyboard::Tab;
                ...
                if (spacePressed && gCoordinator.HasComponent<Position>(player)) {
                    ...
                    if (!networkMode) {
                        ...
                        if (chargeLevel > 0) {
                            ...
                            CreateMissile(playerPos.x + 99.0f, playerPos.y + 25.0f, true,
                                          chargeLevel);
                        } else {
        ...
        if (!inMenu && eng::engine::Keyboard::isKeyPressed(eng::engine::Key::Space)) {
            ...
            if (spaceHoldTime >= chargeStartTime && !hasChargingEffect &&
                gCoordinator.HasComponent<Position>(player)) {
                auto& playerPos = gCoordinator.GetComponent<Position>(player);

                ECS::Entity chargeEffect = gCoordinator.CreateEntity();
                RegisterEntity(chargeEffect);
                gCoordinator.AddComponent(chargeEffect,
                                          Position{playerPos.x + 99.0f, playerPos.y - 5.0f});
            ...
            if (enemySpawnTimer >= enemySpawnInterval) {
                ...
                if (enemiesConfig.valid()) {
                    ...
                    if (enemyConfig.valid()) {
                        sol::protected_function createEnemy =
                            lua["Factory"]["CreateEnemyFromConfig"];
                        if (createEnemy.valid()) {
                            ...
                            if (result.valid()) {
                                ...
                                if (enemy != 0) {
                                    ...
                                    std::cout << "[Game] Spawned " << enemyName
                                              << " at Y=" << randomY << std::endl;
                                }
                            } else {
                                // Lua factory failed at runtime - fallback to C++ CreateEnemy
                                std::cerr
                                    << "[Game] WARNING: Lua CreateEnemy failed, using C++ fallback"
                                    << std::endl;
                            ...
                            std::cerr << "[Game] WARNING: Lua factory not available, using C++ "
                                         "fallback for enemy spawn"
                                      << std::endl;
                            ECS::Entity fallback = CreateEnemy(1920.0f, randomY, enemyType);
                            (void)fallback;
                        }
                ...
                enemySpawnInterval =
                    1.5f + static_cast<float>(rand() % 20) / 10.0f;  // 1.5 to 3.5 seconds
            }
                    ...
                    if (weaponsConfig.valid()) {
                        ...
                        if (weaponTable.valid()) {
                            ...
                            if (isAimed) {
                                ...
                                sol::protected_function createProj =
                                    lua["Factory"]["CreateProjectileFromWeapon"];
                                if (createProj.valid()) {
                                    auto result =
                                        createProj(weapon.weaponType, pos.x - 30.0f, pos.y, false,
                                                   static_cast<int>(entity), 1);
                                    if (result.valid()) {
                                        ECS::Entity proj = result;
                                        if (proj != 0 &&
                                            gCoordinator.HasComponent<Velocity>(proj)) {
                                            auto& vel = gCoordinator.GetComponent<Velocity>(proj);
                                            float speed =
                                                std::sqrt(vel.dx * vel.dx + vel.dy * vel.dy);
                                ...
                                float angleStep =
                                    (projCount > 1) ? (spreadAngle / (projCount - 1)) : 0.0f;
                                ...
                                for (int i = 0; i < projCount; i++) {
                                    ...
                                    if (createProj.valid()) {
                                        auto result =
                                            createProj(weapon.weaponType, pos.x - 30.0f, pos.y,
                                                       false, static_cast<int>(entity), 1);
                                        if (result.valid()) {
                                            ECS::Entity proj = result;
                                            if (proj != 0 &&
                                                gCoordinator.HasComponent<Velocity>(proj)) {
                                                auto& vel =
                                                    gCoordinator.GetComponent<Velocity>(proj);
                                                float baseSpeed =
                                                    std::sqrt(vel.dx * vel.dx + vel.dy * vel.dy);
        ...
        if (!inMenu) {
            ...
            if (networkMode) {
                ...
                movementPatternSystem->SetPlayerEntity(
                    player);  // Update player reference for chase patterns
            ...
            if (player != 0 && playerCreated) {
                ...
                if (gCoordinator.HasComponent<ShootEmUp::Components::Score>(player)) {
                    ...
                    if (gameFontLoaded) {
                        playerScoreUI.UpdateScore(score.currentScore, score.comboMultiplier,
...
void Game::PlayMusic(const std::string& musicName, bool loop) {
    ...
    std::cout << "[Audio] ♪ Playing: " << musicName << " (Volume: " << currentMusicVolume
...
void Game::FadeToMusic(const std::string& musicName, float duration) {
    if (currentMusicSound && currentMusicSound->getStatus() == eng::engine::Sound::Playing) {
        ...
        std::cout << "[Audio] Starting fade to: " << musicName << " (duration: " << duration << "s)"
                  << std::endl;
    } else {
...
void Game::LoadDifficulty(const std::string& difficulty) {
    std::string diffPath =
        ResolveAssetPath("game/assets/scripts/difficulty_game/difficulty_" + difficulty + ".lua");
...
void Game::LoadUserSettings() {
    ...
    if (file.is_open()) {
        ...
        std::cout << "[Settings]   Music: " << currentMusicVolume << "%, SFX: " << currentSFXVolume
                  << "%" << std::endl;
    } else {
