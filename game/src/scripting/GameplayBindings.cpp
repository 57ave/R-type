...
bool GameplayBindings::LoadMasterConfig(sol::state& lua, const std::string& basePath) {
    ...
    try {
        ...
        if (!result.valid()) {
            ...
            std::cerr << "[GameplayBindings] Failed to load master config: " << err.what()
                      << std::endl;
            return false;
        }
        ...
        std::cerr << "[GameplayBindings] Exception loading master config: " << e.what()
                  << std::endl;
        return false;
    }
}
...
LuaWeaponConfig GameplayBindings::GetWeaponConfig(sol::state& lua, const std::string& weaponName,
                                                  int level) {
    LuaWeaponConfig config;
