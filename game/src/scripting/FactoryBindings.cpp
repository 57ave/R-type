...
void FactoryBindings::RegisterFactories(
    ...
    FactoryContext* ctx =
        new FactoryContext{coordinator, textures, spriteList, &lua, registerEntityCallback};
    ...
    std::function<ECS::Entity(float, float)> createBasic = [ctx](float x, float y) -> ECS::Entity {
        ...
        ECS::Entity e =
            EnemyFactory::CreateBasicEnemy(*ctx->coordinator, x, y, enemyTexture, *ctx->spriteList);
        if (e != 0 && ctx->registerEntity)
            ctx->registerEntity(e);
        return e;
    };
    ...
    std::function<ECS::Entity(float, float)> createZigZag = [ctx](float x, float y) -> ECS::Entity {
        ...
        if (!enemyTexture)
            return 0;
        ECS::Entity e = EnemyFactory::CreateZigZagEnemy(*ctx->coordinator, x, y, enemyTexture,
                                                        *ctx->spriteList);
        if (e != 0 && ctx->registerEntity)
            ctx->registerEntity(e);
        return e;
    ...
    std::function<ECS::Entity(float, float)> createSineWave = [ctx](float x,
                                                                    float y) -> ECS::Entity {
        auto* enemyTexture = ctx->textures["enemy"];
        if (!enemyTexture)
            return 0;
        ECS::Entity e = EnemyFactory::CreateSineWaveEnemy(*ctx->coordinator, x, y, enemyTexture,
                                                          *ctx->spriteList);
        if (e != 0 && ctx->registerEntity)
            ctx->registerEntity(e);
        return e;
    ...
    std::function<ECS::Entity(float, float)> createKamikaze = [ctx](float x,
                                                                    float y) -> ECS::Entity {
        auto* enemyTexture = ctx->textures["enemy"];
        if (!enemyTexture)
            return 0;
        ECS::Entity e = EnemyFactory::CreateKamikazeEnemy(*ctx->coordinator, x, y, enemyTexture,
                                                          *ctx->spriteList);
        if (e != 0 && ctx->registerEntity)
            ctx->registerEntity(e);
        return e;
    ...
    std::function<ECS::Entity(float, float)> createTurret = [ctx](float x, float y) -> ECS::Entity {
        ...
        if (!enemyTexture)
            return 0;
        ECS::Entity e = EnemyFactory::CreateTurretEnemy(*ctx->coordinator, x, y, enemyTexture,
                                                        *ctx->spriteList);
        if (e != 0 && ctx->registerEntity)
            ctx->registerEntity(e);
        return e;
    ...
    std::function<ECS::Entity(float, float)> createBoss = [ctx](float x, float y) -> ECS::Entity {
        ...
        if (!enemyTexture)
            return 0;
        ECS::Entity e =
            EnemyFactory::CreateBossEnemy(*ctx->coordinator, x, y, enemyTexture, *ctx->spriteList);
        if (e != 0 && ctx->registerEntity)
            ctx->registerEntity(e);
        return e;
    };
    ...
    std::function<ECS::Entity(std::string, float, float)> createEnemy =
        [ctx](std::string enemyType, float x, float y) -> ECS::Entity {
        auto* enemyTexture = ctx->textures["enemy"];
        if (!enemyTexture)
            return 0;

        ECS::Entity e = EnemyFactory::CreateEnemy(*ctx->coordinator, enemyType, x, y, enemyTexture,
                                                  *ctx->spriteList);
        if (e != 0 && ctx->registerEntity)
            ctx->registerEntity(e);
        return e;
    ...
    auto CreateProjectileFromWeaponConfig = [ctx](std::string weaponName, float x, float y,
                                                  bool isPlayer, int ownerId, int level = 1,
                                                  bool charged = false) -> ECS::Entity {
    ...
    std::function<ECS::Entity(float, float, bool, int)> createNormalProj =
        [ctx, CreateProjectileFromWeaponConfig](float x, float y, bool isPlayer,
                                                int ownerId) -> ECS::Entity {
    ...
    std::function<ECS::Entity(float, float, int, bool, int)> createChargedProj =
        [ctx, CreateProjectileFromWeaponConfig](float x, float y, int chargeLevel, bool isPlayer,
                                                int ownerId) -> ECS::Entity {
        std::string weaponName = isPlayer ? "single_shot" : "enemy_bullet";
        return CreateProjectileFromWeaponConfig(weaponName, x, y, isPlayer, ownerId, chargeLevel,
                                                true);
    };
    ...
    std::function<ECS::Entity(float, float, int, bool, int)> createPiercingProj =
        [ctx, CreateProjectileFromWeaponConfig](float x, float y, int maxPierce, bool isPlayer,
                                                int ownerId) -> ECS::Entity {
    ...
    std::function<ECS::Entity(std::string, float, float, bool, int, int)> createProjFromWeapon =
        [ctx](std::string weaponName, float x, float y, bool isPlayer, int ownerId,
              int level) -> ECS::Entity {
        sol::state_view lua(*ctx->lua);
        sol::table weapons = lua["WeaponsConfig"];
        if (!weapons.valid()) {
            std::cerr << "[Factory] WeaponsConfig not found!" << std::endl;
        ...
        if (!proj.valid()) {
            std::cerr << "[Factory] Weapon '" << weaponName << "' has no projectile spec"
                      << std::endl;
            return 0;
        }
        ...
        if (!texPath.empty()) {
            ...
            if (it == ctx->textures.end() || it->second == nullptr) {
                ...
                if (!assetBase.empty()) {
                    if (assetBase.back() != '/' && assetBase.back() != '\\')
                        assetBase += '/';
            ...
            if (it2 != ctx->textures.end())
                tex = it2->second;
        }
        if (!tex)
            tex = ctx->textures["missile"];  // fallback
        if (!tex) {
        ...
        try {
            ...
            if (spriteTable.valid()) {
                ...
                if (!texPath.empty()) {
                    ...
                    if (ctx->textures.find(texPath) == ctx->textures.end() ||
                        ctx->textures[texPath] == nullptr) {
                        auto* newTex = new eng::engine::rendering::sfml::SFMLTexture();
                        // Try several candidate paths: ASSET_BASE_PATH + game/assets/,
                        // ASSET_BASE_PATH + texPath, texPath
                        std::string assetBase = lua["ASSET_BASE_PATH"].get_or(std::string(""));
                        std::vector<std::string> candidates;
                        if (!assetBase.empty()) {
                        ...
                        for (auto& p : candidates) {
                            if (newTex->loadFromFile(p)) {
                                std::cout << "[Factory] Loaded dynamic texture for enemy: " << p
                                          << std::endl;
                                loaded = true;
                                break;
                            }
                        ...
                        if (!loaded) {
                            std::cerr << "[Factory] Failed to load enemy texture: " << texPath
                                      << std::endl;
                            delete newTex;
                        } else {
                            ctx->textures[texPath] = newTex;
            ...
            std::cerr << "[Factory] Exception while loading enemy texture: " << e.what()
                      << std::endl;
        }
        ECS::Entity e = EnemyFactory::CreateEnemyFromLuaConfig(*ctx->coordinator, x, y, config,
                                                               ctx->textures, *ctx->spriteList);
        if (e != 0 && ctx->registerEntity)
            ctx->registerEntity(e);
        return e;
    };
