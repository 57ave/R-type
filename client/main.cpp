...
class Player
{
    ...
    public:
        Player() : speed(500.0f), animationTime(0.0f), currentColumn(2),
                   targetColumn(2), playerLine(0), transitionSpeed(0.15f) {}
...
class ChargedShootEffect
{
    ...
    public:
        ChargedShootEffect() : animationTime(0.0f), frameTime(0.08f), currentFrame(0), finished(false) {}
        ...
        bool isFullyCharged() const { return currentFrame >= 5; } // Considéré comme chargé à partir de la frame 6/8

        // Calcule le niveau de charge (1-5) en fonction de la progression de l'animation
        int getChargeLevel() const
        {
            if (currentFrame < 2) return 1;      // Frames 0-1: Niveau 1 (ligne 5)
            else if (currentFrame < 3) return 2; // Frame 2: Niveau 2 (ligne 6)
            else if (currentFrame < 5) return 3; // Frames 3-4: Niveau 3 (ligne 7)
            else if (currentFrame < 6) return 4; // Frame 5: Niveau 4 (ligne 8)
            else return 5;                        // Frames 6-7: Niveau 5 (ligne 9)
        }
};
...
class ChargedMissile
{
    ...
    public:
        ChargedMissile() : speed(1500.0f), animationTime(0.0f), frameTime(0.1f), currentFrame(0), chargeLevel(1) {}
        ...
        void animate(float deltaTime)
        {
            ...
            if (animationTime >= frameTime)
            {
                ...
                sf::IntRect newRect(data.xPos + (currentFrame * data.frameWidth), data.yPos, data.width, data.height);
                sprite.setTextureRect(newRect);
            }
        }
            ...
            if (ev.type == sf::Event::KeyReleased && ev.key.code == sf::Keyboard::Space)
            {
                if (spacePressed)
                {
                    ...
                    else
                    {
                        ...
                        sf::Vector2f playerPos = player.getPosition();
                        sf::FloatRect playerBounds = player.getBounds();
                        sf::Vector2f effectPos(playerPos.x + playerBounds.width - 10, playerPos.y + 10);
