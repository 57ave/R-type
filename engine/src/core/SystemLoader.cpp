...
SystemLoader::SystemLoader(ECS::Coordinator* coordinator) : m_Coordinator(coordinator) {}
...
std::shared_ptr<ECS::System> SystemLoader::LoadSystem(const std::string& libPath,
    ...
    std::cout << "[SystemLoader] Loaded system '" << systemName << "' from: " << libPath
              << std::endl;
    return system;
}
