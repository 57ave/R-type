...
ScopedProfiler::ScopedProfiler(const std::string& sectionName)
    : _sectionName(sectionName), _startTime(std::chrono::high_resolution_clock::now()) {
...
Profiler::Profiler() : _enabled(true), _initialized(false) {}
...
size_t Profiler::estimateMemoryUsage() const {
    ...
    if (task_info(mach_task_self(), MACH_TASK_BASIC_INFO, (task_info_t)&info, &size) ==
        KERN_SUCCESS) {
...
std::string Profiler::generateReport() const {
    ...
    report << "Average FPS: "
           << ((_totalFrameTime > 0) ? (1000.0 * _frameCount / _totalFrameTime) : 0.0) << "\n";
    report << "Frame Time: " << _currentFrameTimeMs << " ms (min: " << _minFrameTimeMs
           << ", max: " << _maxFrameTimeMs << ")\n";
    ...
    if (!_sections.empty()) {
        ...
        std::vector<std::pair<std::string, ProfileSection>> sortedSections(_sections.begin(),
                                                                           _sections.end());
        ...
        for (const auto& [name, section] : sortedSections) {
            report << "  " << name << ": " << section.avgTimeMs << " ms avg, " << section.callCount
                   << " calls, " << section.totalTimeMs << " ms total\n";
        }
    }

    if (_networkStats.packetsSent > 0 || _networkStats.packetsReceived > 0) {
        report << "\n--- Network Stats ---\n";
        report << "  Packets Sent: " << _networkStats.packetsSent << " ("
               << (_networkStats.bytesSent / 1024) << " KB)\n";
        report << "  Packets Received: " << _networkStats.packetsReceived << " ("
               << (_networkStats.bytesReceived / 1024) << " KB)\n";
        report << "  Latency: " << _networkStats.latencyMs
               << " ms (jitter: " << _networkStats.jitterMs << " ms)\n";
    }
