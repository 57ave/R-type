...
bool DevConsole::init(const std::string& fontPath) {
    if (fontPath.empty()) {
        ...
        std::vector<std::string> fontPaths = {
            ...
            // Relative paths
            "assets/fonts/arial.ttf", "../assets/fonts/arial.ttf", "../../assets/fonts/arial.ttf"};
...
void DevConsole::registerBuiltinCommands() {
    ...
    registerCommand("loglevel", "Set log level (debug/info/warning/error)", "loglevel <level>",
                    [](const std::vector<std::string>& args) -> std::string {
                        ...
                        if (levelStr == "debug")
                            ...
                            return "Invalid log level. Use: debug, info, warning, error";
    ...
    // Profiler command
    registerCommand(
        "profiler", "Toggle profiler overlay or show report", "profiler [on|off|report]",
        [](const std::vector<std::string>& args) -> std::string {
            auto& profiler = Profiler::getInstance();
            if (args.size() < 2) {
...
void DevConsole::render(sf::RenderWindow& window) {
    if (!_initialized || _animationProgress <= 0.0f)
        return;
...
void DevConsole::registerCommand(const std::string& name, const std::string& description,
