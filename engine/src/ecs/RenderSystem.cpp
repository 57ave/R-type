...
eng::engine::rendering::Transform RenderSystem::ToRenderingTransform(
    const eng::engine::ECS::Transform& ecsTransform) {
...
void RenderSystem::EnsureSpriteLoaded(eng::engine::ECS::Sprite& sprite) {
    ...
    try {
        ...
        if (!sprite.sprite) {
            std::cerr << "[RenderSystem] Warning: Failed to load sprite: " << sprite.texturePath
                      << std::endl;
        }
