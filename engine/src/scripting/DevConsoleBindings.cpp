...
void DevConsoleBindings::Register(sol::state& lua, rtype::core::DevConsole* console) {
    ...
    consoleTable.set_function("register", [&lua](const std::string& name,
                                                 const std::string& description,
                                                 const std::string& usage, sol::function callback) {
        if (!s_Console) {
            LOG_ERROR("SCRIPTING",
                      "Cannot register command '" + name + "': console not initialized");
            return;
        }
        ...
        auto cppCallback = [callback, name](const std::vector<std::string>& args) -> std::string {
            try {
                ...
                if (!result.valid()) {
                    ...
                    std::string errorMsg =
                        "Lua error in command '" + name + "': " + std::string(err.what());
                ...
                std::string errorMsg =
                    "Exception in Lua command '" + name + "': " + std::string(e.what());
                LOG_ERROR("SCRIPTING", errorMsg);
                return errorMsg;
            }
        };
    ...
    consoleTable.set_function("register_simple", [&lua](const std::string& name,
    ...
    consoleTable.set_function("is_open",
                              []() -> bool { return s_Console ? s_Console->isOpen() : false; });

    LOG_INFO("SCRIPTING", "DevConsole bindings registered");
}

bool DevConsoleBindings::LoadCommandsFile(sol::state& lua, rtype::core::DevConsole* console,
