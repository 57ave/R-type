...
void UISystem::HandleEvent(const eng::engine::InputEvent& event) {
    ...
    if (event.type == EventType::MouseMoved) {
        ...
        m_hoveredEntity =
            GetEntityAtPosition(static_cast<float>(m_mouseX), static_cast<float>(m_mouseY));
    ...
    // Handle keyboard events
        ...
        if (event.key.code == Key::Up) {
            ...
            if (m_focusedInputField != 0 && m_coordinator &&
                m_coordinator->HasComponent<Components::UIInputField>(m_focusedInputField)) {
                auto& input =
                    m_coordinator->GetComponent<Components::UIInputField>(m_focusedInputField);
...
void UISystem::HandleTextInput(char character) {
    ...
    if (!m_coordinator->HasComponent<Components::UIInputField>(m_focusedInputField))
        return;
    ...
    if (input.text.length() >= input.maxLength)
        return;
    if (input.numbersOnly && (character < '0' || character > '9'))
        return;
    if (input.alphanumericOnly && !std::isalnum(character))
        return;
    if (!input.allowedCharacters.empty() &&
        input.allowedCharacters.find(character) == std::string::npos)
        return;
...
ECS::Entity UISystem::GetEntityAtPosition(float x, float y) const {
    ...
    for (auto entity : mEntities) {
        try {
            ...
            std::cerr << "[UISystem] Error checking entity " << entity << ": " << e.what()
                      << std::endl;
        }
    }
...
void UISystem::HandleMouseInput() {
    ECS::Entity entityUnderMouse =
        GetEntityAtPosition(static_cast<float>(m_mouseX), static_cast<float>(m_mouseY));
    // Handle click (on mouse down)
    bool clicked = m_mousePressed && !m_mousePreviouslyPressed;
    if (clicked) {
        std::cout << "[UISystem] Mouse click at (" << m_mouseX << "," << m_mouseY
                  << ") entityUnderMouse=" << entityUnderMouse << std::endl;
        ...
        if (m_openDropdown != 0) {
            ...
            if (dropdown.isOpen &&
                m_coordinator->HasComponent<Components::UIElement>(m_openDropdown)) {
                ...
                for (size_t i = 0; i < dropdown.options.size(); ++i) {
                    ...
                    if (m_mouseY >= optTop && m_mouseY < optBot && m_mouseX >= element.x &&
                        m_mouseX < element.x + element.width) {
        ...
        if (entityUnderMouse != 0) {
            if (m_coordinator->HasComponent<Components::UIButton>(entityUnderMouse)) {
                ...
                if (m_coordinator->HasComponent<Components::UIElement>(entityUnderMouse)) {
                    const auto& elem =
                        m_coordinator->GetComponent<Components::UIElement>(entityUnderMouse);
                    std::cout << "[UISystem] Click on entity " << entityUnderMouse << " menu='"
                              << elem.menuGroup << "' visible=" << elem.visible
                              << " interactable=" << elem.interactable << std::endl;
                }
                std::cout << "[UISystem] UIButton callback='" << button.onClickCallback
                          << "' enabled=" << button.enabled << std::endl;
            ...
            if (m_coordinator->HasComponent<Components::UICheckbox>(entityUnderMouse)) {
                auto& checkbox =
                    m_coordinator->GetComponent<Components::UICheckbox>(entityUnderMouse);
                if (checkbox.enabled) {
                    checkbox.checked = !checkbox.checked;
                    if (!checkbox.onChangeCallback.empty()) {
                        CallLuaValueCallback(checkbox.onChangeCallback,
                                             checkbox.checked ? 1.0f : 0.0f);
                        CallCppValueCallback(checkbox.onChangeCallback,
                                             checkbox.checked ? 1.0f : 0.0f);
                    }
                }
            }
            ...
            if (m_coordinator->HasComponent<Components::UIInputField>(entityUnderMouse)) {
                ...
                auto& input =
                    m_coordinator->GetComponent<Components::UIInputField>(entityUnderMouse);
            ...
            if (m_coordinator->HasComponent<Components::UIDropdown>(entityUnderMouse)) {
                auto& dropdown =
                    m_coordinator->GetComponent<Components::UIDropdown>(entityUnderMouse);
                if (!dropdown.isOpen) {
    ...
    if (m_mousePressed) {
        for (auto entity : mEntities) {
            if (m_coordinator->HasComponent<Components::UISlider>(entity)) {
                ...
                bool isUnderMouse =
                    IsPointInRect(static_cast<float>(m_mouseX), static_cast<float>(m_mouseY),
...
void UISystem::CallLuaCallback(const std::string& callbackName) {
    ...
    try {
        ...
        if (func.valid()) {
            std::cout << "[UISystem] Lua function '" << callbackName << "' found, invoking..."
                      << std::endl;
            auto result = func();
            if (!result.valid()) {
                sol::error err = result;
                std::cerr << "[UISystem] Lua error in " << callbackName << ": " << err.what()
                          << std::endl;
            }
        }
    } catch (const std::exception& e) {
        std::cerr << "[UISystem] Exception calling Lua callback " << callbackName << ": "
                  << e.what() << std::endl;
    }
}
