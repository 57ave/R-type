...
void UISystem::DrawRect(float x, float y, float width, float height, 
                        uint32_t fillColor, uint32_t outlineColor, float outlineThickness)
{
    if (!m_window) return;
...
void UISystem::DrawText(const std::string& text, float x, float y, 
                        ...
                        TextAlign align, const std::string& fontId)
{
    if (!m_window) return;
    
    auto* fontPtr = GetFont(fontId);
    if (!fontPtr) fontPtr = GetFont("default");
    if (!fontPtr) return;
...
{
    if (!m_coordinator || !m_window) return;
...
void UISystem::RenderPanel(ECS::Entity entity)
{
    ...
    // Draw background
    DrawRect(element.x + offsetX, element.y + offsetY, 
             element.width * scale, element.height * scale,
             bgColor, panel.borderColor, panel.borderThickness);

    // Draw title bar if enabled
    if (panel.showTitleBar) {
        DrawRect(element.x + offsetX, element.y + offsetY,
                 element.width * scale, panel.titleBarHeight,
                 panel.titleBarColor, 0, 0);

        // Draw title text
        DrawText(panel.title, 
                 element.x + offsetX + element.width * scale / 2.0f,
                 element.y + offsetY + 10.0f,
                 20, 0xFFFFFFFF, TextAlign::Center);
    }
}
...
void UISystem::RenderButton(ECS::Entity entity, float dt)
{
    ...
    if (button.state == Components::UIButton::State::Hovered || 
        button.state == Components::UIButton::State::Selected ||
    ...
    if ((bgColor & 0xFF) > 0) {
        DrawRect(element.x + offsetX, element.y + offsetY,
                 ...
                 bgColor, button.borderColor, borderThickness);
    }
...
void UISystem::RenderText(ECS::Entity entity, float dt)
{
    ...
    if (textComp.shadow) {
        DrawText(textComp.content, 
                 element.x + textComp.shadowOffsetX, 
                 element.y + textComp.shadowOffsetY,
                 static_cast<unsigned int>(textComp.fontSize * scale),
                 textComp.shadowColor, align, textComp.fontId);
    }

    // Draw main text
    DrawText(textComp.content, element.x, element.y,
             static_cast<unsigned int>(textComp.fontSize * scale),
             textComp.color, align, textComp.fontId);
}
...
void UISystem::RenderSlider(ECS::Entity entity)
{
    ...
    uint32_t handleColor = slider.isHovered || slider.isDragging ? 
                           slider.handleHoverColor : slider.handleColor;
    
    DrawRect(handleX, handleY, slider.handleWidth, slider.handleHeight, 
             handleColor, 0x00FFFFFF, 2.0f);
    ...
    if (slider.showValue) {
        ...
        DrawText(displayValue, element.x + element.width, element.y - 25.0f, 
                 16, 0xFFFFFFFF, TextAlign::Right);
    }
}
...
void UISystem::RenderInputField(ECS::Entity entity, float dt)
{
    ...
    DrawRect(element.x, element.y, element.width, element.height,
             input.backgroundColor, borderColor, input.borderThickness);
    ...
    DrawText(displayText, element.x + input.padding, element.y + element.height / 2.0f - 10.0f,
             20, textColor, TextAlign::Left);
...
void UISystem::RenderCheckbox(ECS::Entity entity)
{
    ...
    if (checkbox.checked) {
        ...
        DrawRect(element.x + padding, element.y + padding,
                 checkbox.boxSize - padding * 2.0f, checkbox.boxSize - padding * 2.0f,
                 checkbox.checkColor, 0, 0);
    }

    // Draw label
    if (!checkbox.label.empty()) {
        float labelX = checkbox.labelOnRight ? 
                       element.x + checkbox.boxSize + checkbox.labelSpacing :
                       element.x - checkbox.labelSpacing;
        TextAlign align = checkbox.labelOnRight ? TextAlign::Left : TextAlign::Right;
        DrawText(checkbox.label, labelX, element.y + checkbox.boxSize / 2.0f - 10.0f,
                 20, 0xFFFFFFFF, align);
    }
}
...
void UISystem::RenderDropdown(ECS::Entity entity)
{
    ...
    DrawRect(element.x, element.y, element.width, element.height,
             dropdown.backgroundColor, borderColor, dropdown.borderThickness);

    // Draw selected text
    DrawText(dropdown.getSelectedText(), element.x + 10.0f, element.y + element.height / 2.0f - 10.0f,
             20, dropdown.textColor, TextAlign::Left);

    // Draw arrow
    if (dropdown.showArrow) {
        std::string arrow = dropdown.isOpen ? "^" : "v";
        DrawText(arrow, element.x + element.width - 20.0f, element.y + element.height / 2.0f - 10.0f,
                 20, dropdown.textColor, TextAlign::Right);
    }
    ...
    if (dropdown.isOpen) {
        ...
        for (size_t i = 0; i < dropdown.options.size(); ++i) {
            ...
            DrawRect(element.x, optionY, element.width, dropdown.optionHeight,
                     optBg, dropdown.borderColor, 1.0f);

            DrawText(dropdown.options[i], element.x + 10.0f, optionY + dropdown.optionHeight / 2.0f - 9.0f,
                     18, dropdown.textColor, TextAlign::Left);
