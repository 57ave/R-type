...
SoundBuffer::SoundBuffer() : m_impl(std::make_unique<internal::SoundBufferImpl>()) {}
...
Sound::Sound() : m_impl(std::make_unique<internal::SoundImpl>()) {}

Sound::~Sound() = default;

void Sound::setBuffer(const SoundBuffer& buffer) {
    const sf::SoundBuffer* sfBuffer =
        static_cast<const sf::SoundBuffer*>(buffer.getInternalBuffer());
    if (sfBuffer) {
...
AudioManager::AudioManager() : m_globalVolume(100.0f) {}
