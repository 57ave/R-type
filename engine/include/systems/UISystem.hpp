...
public:
    ...
    ECS::Entity CreateButton(float x, float y, float width, float height, const std::string& text,
                             const std::string& callback, const std::string& menuGroup = "");
    ECS::Entity CreateText(float x, float y, const std::string& text, unsigned int fontSize = 24,
                           uint32_t color = 0xFFFFFFFF, const std::string& menuGroup = "");
    ECS::Entity CreateSlider(float x, float y, float width, float minVal, float maxVal,
...
private:
    ...
    void DrawRect(float x, float y, float width, float height, uint32_t fillColor,
                  uint32_t outlineColor = 0, float outlineThickness = 0);
    void DrawText(const std::string& text, float x, float y, unsigned int fontSize, uint32_t color,
    ...
    // Font management
    std::unordered_map<std::string, std::unique_ptr<eng::engine::rendering::sfml::SFMLFont>>
        m_fonts;
