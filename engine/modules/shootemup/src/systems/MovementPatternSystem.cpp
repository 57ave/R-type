...
void MovementPatternSystem::Update(float dt) {
    ...
    for (auto entity : mEntities) {
        ...
        if (pattern.patternType == "straight") {
            ...
            pos.x -= pattern.speed * dt;
            pos.y = pattern.startY +
                    pattern.amplitude * std::sin(pattern.frequency * pattern.timeAlive);
        } else if (pattern.patternType == "zigzag") {
            // Zigzag movement
            pos.x -= pattern.speed * dt;
            pos.y = pattern.startY +
                    pattern.amplitude * std::sin(pattern.frequency * pattern.timeAlive * 2.0f);
