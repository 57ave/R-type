...
void BossSystem::UpdateMovement(ECS::Entity boss, float dt) {
    ...
    if (bossComp.movementPattern == "hover") {
        ...
        float hoverOffset =
            std::sin(movementTime * bossComp.hoverFrequency) * bossComp.hoverAmplitude;
...
void BossSystem::UpdatePhase(ECS::Entity boss) {
    ...
    if (newPhase != bossComp.currentPhase) {
        ...
        std::cout << "[BossSystem] " << bossComp.bossName << " entered phase " << newPhase
                  << std::endl;
...
std::string BossSystem::SelectNextAttack(const Components::Boss& boss) {
    ...
    int phaseIndex =
        std::min(boss.currentPhase - 1, static_cast<int>(boss.phasePatterns.size()) - 1);
    if (phaseIndex < 0)
        phaseIndex = 0;
...
void BossSystem::ExecuteChargeAttack(ECS::Entity boss, float speed) {
    ...
    if (!coordinator_->HasComponent<Velocity>(boss))
        return;
...
float BossSystem::GetAngleToPlayer(float bossX, float bossY) {
    if (playerEntity_ == 0)
        return 180.0f;  // Default: shoot left

    if (!coordinator_->HasComponent<Position>(playerEntity_))
        return 180.0f;
...
void BossPartSystem::UpdatePartAttacks(float dt) {
    for (auto& pair : bossParts_) {
        ...
        if (partComp.isDestroyed || !partComp.canAttack)
            continue;
