...
public:
    ...
    using MinionSpawnCallback =
        std::function<ECS::Entity(const std::string& enemyType, float x, float y)>;
...
public:
    ...
    // Callbacks
    using PartDestroyedCallback =
        std::function<void(ECS::Entity boss, ECS::Entity part, const std::string& partType)>;
