        ...
        while (gameRunning_) {
            ...
            if (statsClock_.getElapsedTime() >= statsInterval) {
                ...
                std::cout << "[NetworkStats] Sent: " << sent << " B/s  Received: " << recv << " B/s"
                          << "  TotalSent: " << bytesSentTotal_
                          << " B  TotalRecv: " << bytesReceivedTotal_ << " B" << std::endl;
            }
...
private:
    ...
    void predictPlayerMovement(ServerEntity& entity) {
        if (entity.type == EntityType::ENTITY_PLAYER) {
            ...
            if (itTime != lastPlayerInputTime_.end()) {
                ...
                auto elapsedMs =
                    std::chrono::duration_cast<std::chrono::milliseconds>(now - itTime->second)
                        .count();
                ...
                if (elapsedMs <= predictionWindowMs) {
                    ...
                    if (inputMask & (1 << 0))
                        entity.vy = -speed;  // Up
                    if (inputMask & (1 << 1))
                        entity.vy = speed;  // Down
                    if (inputMask & (1 << 2))
                        entity.vx = -speed;  // Left
                    if (inputMask & (1 << 3))
                        entity.vx = speed;  // Right
                    // We intentionally DO NOT spawn missiles here — only spawn on receipt of input
                    // packet earlier
                }
            }
        }
    ...
    void sendWorldSnapshot(bool forceFull = false) {
        ...
        auto changed = [](const EntityState& a, const EntityState& b) {
            ...
            if (a.type != b.type)
                return true;
            if (std::abs(a.x - b.x) > posEps)
                return true;
            if (std::abs(a.y - b.y) > posEps)
                return true;
            if (std::abs(a.vx - b.vx) > velEps)
                return true;
            if (std::abs(a.vy - b.vy) > velEps)
                return true;
            if (a.hp != b.hp)
                return true;
            if (a.playerLine != b.playerLine)
                return true;
            if (a.playerId != b.playerId)
                return true;
            if (a.chargeLevel != b.chargeLevel)
                return true;
            if (a.enemyType != b.enemyType)
                return true;
            if (a.projectileType != b.projectileType)
                return true;
            return false;
        };
        ...
        if (hasPlayingRooms) {
            for (auto& [roomId, room] : allRooms) {
                ...
                // Build a list of entities that are new or changed compared to last snapshot for
                // this room
                ...
                for (const auto* entity : snapshotEntities) {
                    ...
                    if (forceFull || isNew || isChanged) {
                        ...
                        header.entityCount++;
                        // update cache immediately so repeated comparisons in same frame are
                        // consistent
